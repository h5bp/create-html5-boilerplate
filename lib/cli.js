...
const servers = [
  ...
  { value: "gae", title: "Google App Engine (GAE)" },
  { value: "iis", title: "Internet Information Services (IIS)" },
  { value: "lighttpd", title: "lighttpd" },
  { value: "nginx", title: "Nginx" },
  { value: "node", title: "Node.js" },
];
const serverURLs = {
  apache:
    "https://raw.githubusercontent.com/h5bp/server-configs-apache/main/dist/.htaccess",
  gae: "https://github.com/h5bp/server-configs-gae",
  iis: "https://github.com/h5bp/server-configs-iis",
  lighttpd: "https://github.com/h5bp/server-configs-lighttpd",
  nginx: "https://github.com/h5bp/server-configs-nginx",
  node: "https://github.com/h5bp/server-configs-node",
};
...
const onLoad = async (targetDir, version, argv) => {
  ...
  if (!argv.servers) {
    servers.forEach(({ title, value }) => {
      const text = `${title} (${value})`;
      serverListMap[text] = value;
      serverListOut.push(text);
    });
    langListOut.splice(1, 0, "Enter custom");
  }
  spinner.stop();
  const {
    ...
    },
    {
      ...
      source: async (answers, input = "") =>
        fuzzy.filter(input, serverListOut).map(({ original }) => original),
    },
  ]);
  spinner.start();
  ...
  try {
    ...
    if (addServerConfigs) {
      const choice = whichServerTech.match(/\((.*)\)/);
      const server = choice[1];
      console.log(`let's get ${server} from ${serverURLs[server]}`);
      if (server === "apache") {
        wget({ url: serverURLs[server] });
      }
    }
    return;
  } catch (err) {
